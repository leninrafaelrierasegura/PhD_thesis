...
# 
# pems_clipped1 <- pems_clipped[1,]
# pems_clipped2 <- pems_clipped[2,]
# pems_clipped3 <- pems_clipped[3,]
# pems_clipped4 <- pems_clipped[4,]
# pems_clipped5 <- pems_clipped[5,]
# 
# 
# split_into_segments <- function(line) {
#   coords <- st_coordinates(line)
#   
#   # build segments between consecutive points
#   segs <- lapply(seq_len(nrow(coords) - 1), function(i) {
#     st_linestring(rbind(coords[i, 1:2], coords[i + 1, 1:2]))
#   })
#   
#   st_sfc(segs, crs = st_crs(line))
# }
# 
# segments_list1 <- split_into_segments(pems_clipped1)
# segments_list2 <- split_into_segments(pems_clipped2)
# segments_list3 <- split_into_segments(pems_clipped3)
# segments_list4 <- split_into_segments(pems_clipped4)
# segments_list5 <- split_into_segments(pems_clipped5)
# 
# 
# graph <- metric_graph$new(edges = c(segments_list1,
#                                     segments_list2,
#                                     segments_list3, 
#                                     segments_list4,
#                                     segments_list5), longlat = TRUE)
...
p_to_show_geodesic_dist <- graph$plot(vertex_size = 2, 
                    vertex_color = "blue",
                    edge_color = "black",
                    p = psmall,
                    edge_width = 1) +
...
ggsave(here::here("data_files/p_to_show_geodesic_dist.png"), plot = p_to_show_geodesic_dist, width = dist_x*ratt, height = dist_y*ratt, dpi = 300)
```
...
# Save temporarily
ggsave(here::here("data_files/map_with_square.png"), plot = p_spaguetti, width = 6, height = 6, dpi = 300)
...
layout(font = list(family = "Palatino"),
            ...
            scene = list(
              xaxis = list(title = list(text = "Longitude", font = list(color = colaxnn)),  
                           tickfont = list(color = colaxnn),  range = x_range),
              yaxis = list(title = list(text = "Latitude", font = list(color = colaxnn)),  
                           tickfont = list(color = colaxnn), range = y_range),
              zaxis = list(title = list(text = "", font = list(color = colaxnn)),  
                           tickfont = list(color = colaxnn), range = c(0,z_range[2])),
