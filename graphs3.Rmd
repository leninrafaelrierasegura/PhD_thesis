...
\end{align*}
In particular, $\hat{f} \in C^1([0,\ell_1+\ell_2])$, and therefore $f$ has a continuous first derivative across the vertex $v$. See Figure 2 below. Moreover, the edgewise derivatives must agree at $v$, that is, we must have $f'_{e_1}(\ell_1) = \hat{f}'(v) = f'_{e_2}(0)$, which is the case since
\begin{align*}
...
```{r}
sizeref <- 0.1
pf <- graph$plot_function(X = f, vertex_size = 5, edge_width = 5, line_width = 5, type = "plotly") |> 
  add_trace(
  type = "cone",
  x = ym1,
  y = xm1,
...
pdf <- graph$plot_function(X = df, vertex_size = 5, edge_width = 5, line_width = 5, type = "plotly") |> 
  add_trace(
  type = "cone",
  x = ym1,
  y = xm1,
...
pddf <- graph$plot_function(X = ddf, vertex_size = 5, edge_width = 5, line_width = 5, type = "plotly") |>
  add_trace(
  type = "cone",
  x = ym1,
  y = xm1,
...
pdddf <- graph$plot_function(X = dddf, vertex_size = 5, edge_width = 5, line_width = 5, type = "plotly") |> 
  add_trace(
  type = "cone",
  x = ym1,
  y = xm1,
...
```{r, fig.height = 7, out.width = "100%", fig.cap = captioner("Function $f = \\{f_e\\}_{e\\in\\mathcal{E}}$ given by $f_{e_1}(s) = t^4$ and $f_{e_2}(s) = (t+\\ell_1)^4$.")}
pf
```
...
```{r, fig.height = 7, out.width = "100%", fig.cap = captioner("Function $f' = \\{f'_e\\}_{e\\in\\mathcal{E}}$ given by $f'_{e_1}(s) = 4t^3$ and $f'_{e_2}(s) = 4(t+\\ell_1)^3$.")}
pdf
```
...
```{r, fig.height = 7, out.width = "100%", fig.cap = captioner("Function $f'' = \\{f''_e\\}_{e\\in\\mathcal{E}}$ given by $f''_{e_1}(s) = 12t^2$ and $f''_{e_2}(s) = 12(t+\\ell_1)^2$.")}
pddf
```
...
```{r, fig.height = 7, out.width = "100%", fig.cap = captioner("Function $f''' = \\{f'''_e\\}_{e\\in\\mathcal{E}}$ given by $f'''_{e_1}(s) = 24t$ and $f'''_{e_2}(s) = 24(t+\\ell_1)$.")}
pdddf
```
...
\end{equation}
which shows that $g$ satisfies the Kirchhoff vertex conditions \eqref{eq:kirchhoff_cond} at vertex $v$. At first glance, the edgewise derivative, $g' = \{g'_{e_1},g'_{e_2}\}$ appears discontinuous at $v$, which might seem contradictory. See Figure 6 below.
...
\end{equation}
so that the second derivative does not change sign under a flip. See Figure 7. In general, for the $k$-th derivative, we have
\begin{equation}
...
```{r}
p_base <- graph$plot_function(X = rep(0, nrow(graph$mesh$VtE)), vertex_size = 5, line_color = "black", edge_width = 5, line_width = 5, type = "plotly")
DF <- f$DF
pf3 <- p_base |>
  add_trace(
  type = "cone",
...
```{r, fig.height = 7, out.width = "100%", fig.cap = captioner("Function $g = \\{g_e\\}_{e\\in\\mathcal{E}}$ given by $g_{e_1}(s) = \\tau^4$ and $g_{e_2}(s) = (\\tau-(\\ell_1+\\ell_2))^4$.")}
pf3
```
...
```{r, fig.height = 7, out.width = "100%", fig.cap = captioner("Function $g' = \\{g'_e\\}_{e\\in\\mathcal{E}}$ given by $g'_{e_1}(s) = 4\\tau^3$ and $g'_{e_2}(s) = 4(\\tau-(\\ell_1+\\ell_2))^3$.")}
pdf3
```
...
```{r, fig.height = 7, out.width = "100%", fig.cap = captioner("Function $g'' = \\{g''_e\\}_{e\\in\\mathcal{E}}$ given by $g''_{e_1}(s) = 12\\tau^2$ and $g''_{e_2}(s) = 12(\\tau-(\\ell_1+\\ell_2))^2$.")}
pddf3
```
...
```{r, fig.height = 7, out.width = "100%", fig.cap = captioner("Function $g''' = \\{g'''_e\\}_{e\\in\\mathcal{E}}$ given by $g'''_{e_1}(s) = 24\\tau$ and $g'''_{e_2}(s) = 24(\\tau-(\\ell_1+\\ell_2))$.")}
pdddf3
```
