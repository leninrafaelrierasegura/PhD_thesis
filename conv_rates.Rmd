...
conv_rates_p <- ggplot() + 
  ...
  labs(x = "$h\\quad(\\log_{10}\\mbox{ scale})$", y = "$\\|\\varrho^{\\alpha} - \\varrho_{h,m}^{\\alpha}\\|_{L_2(\\Gamma\\times \\Gamma)}\\quad(\\log_{10}\\mbox{ scale})$", color = "$\\alpha$") 
...
knitr::include_graphics(here::here("data_files/tikzpic/conv_rates_p_with_mathpazo.pdf"))
```
