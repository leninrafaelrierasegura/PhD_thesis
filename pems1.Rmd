...
# Combine the top row with p1 in a grid
combined_plot <- plot_grid(left_col, g12, labels = NULL, ncol = 2, rel_widths = c(1,2)) 
final_plot <- annotate_figure(combined_plot, left = textGrob("Latitude", rot = 90, vjust = 6, gp = gpar(cex = 0.8)),
                              bottom = textGrob("Longitude", vjust = 0, gp = gpar(cex = 0.8)))
ggsave(here("data_files/sanjosehighways.png"), width = 11.2, height = 5.43, plot = final_plot, dpi = 500)
ggsave("~/Desktop/leninPresentations/data_files/sanjosehighways.png", width = 11.2, height = 5.43, plot = final_plot, dpi = 500)


final_alone_plot <- annotate_figure(g12, left = textGrob("Latitude", rot = 90, vjust = 23, gp = gpar(cex = 0.8)),
                              bottom = textGrob("Longitude", vjust = 0, gp = gpar(cex = 0.8)))




ggsave(here("data_files/sanjosehighways_alone.png"), width = 11.2, height = 5.43, plot = final_alone_plot, dpi = 500)
...
edges_lonlat <- st_transform(mf04p$edges, 4326)
GRAPH <- metric_graph$new(edges_lonlat)

bbox_lonlat <- st_bbox(st_transform(mf04p$edges, 4326))

new_left <- as.numeric(bbox_lonlat["xmin"])
new_bottom <- as.numeric(bbox_lonlat["ymin"])
new_right <- as.numeric(bbox_lonlat["xmax"])
new_top <- as.numeric(bbox_lonlat["ymax"])

p_river <- ggmap(get_stadiamap(
  bbox = c(
    left   = new_left,
    bottom = new_bottom,
    right  = new_right,
...
groad_and_river <- g12new + g_river +
  plot_layout(ncol = 2)

ggsave(here("data_files/sanjosehighways_and_river_network.png"), width = 10, height = 3, plot = groad_and_river, dpi = 500)
```
...
metric_graph$new(sf::st_transform(mf04p$edges, 4326))$plot()
# Plot of San Jose network
metric_graph$new(pems$edges)$plot()
...
```{r, eval = TRUE, fig.width = 11.2, fig.height = 5.43, fig.cap = captioner("Highway network of the city of San Jose in California. The left panels are zoomed-in areas of the panel to the right.")}
knitr::include_graphics(here::here("data_files/sanjosehighways.png"))
```

```{r, eval = TRUE, fig.width = 10, fig.height = 3, fig.cap = captioner("Highway network of the city of San Jose in California and river network.")}
knitr::include_graphics(here::here("data_files/sanjosehighways_and_river_network.png"))
```
