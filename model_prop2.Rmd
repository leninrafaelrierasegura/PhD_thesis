...
Go back to the [About page](about.html). To see how the covariance function changes when the location changes, go to [model_properties1.html](model_prop1.html).
...
Below we consider $\alpha = 0.9, 1.3, 2.1$ and compute the covariance between one point and all other points on the mesh. 
...
```{r, eval = TRUE, fig.width = 13.83, fig.height = 4.5, fig.cap = captioner("Example of covariance functions $r_\\nu(\\cdot)$ for different smoothness parameter $\\nu$ on the `MetricGraph` package's logo.")}
load(here::here("data_files/csmp2.Rdata"))
csmp2
```
...
```{r}
p1 <- logo_graph$plot_function(X = rowfromcov1, vertex_size = 1, plotly = TRUE, edge_color = "black", edge_width = gsw, line_color = "#0000C8", line_width = gsw)
p2 <- logo_graph$plot_function(X = rowfromcov2, vertex_size = 1, plotly = TRUE, edge_color = "black", edge_width = gsw, line_color = "red", line_width = gsw, p = p1)
p3 <- logo_graph$plot_function(X = rowfromcov3, vertex_size = 1, plotly = TRUE, edge_color = "black", edge_width = gsw, line_color = "darkgreen", line_width = gsw, p = p2)
p_B <- p3 %>%
  config(mathjax = 'cdn') %>%
...
```{r, eval = TRUE, fig.height = 7, out.width = "100%", fig.cap = captioner("Example of covariance functions $r_\\nu(\\cdot)$ for different smoothness parameter $\\nu$ on the `MetricGraph` package's logo.")}
