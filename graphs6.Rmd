...
ff <- function(t) 10*abs(sin(-2*t))/3

ff_eval <- ff(s_smooth)

curve_smooth$height <- ff_eval

s_semi_coarse <- seq(0, L, length.out = n_smooth)
ff_s_semi_coarse <- ff(s_semi_coarse)
y_semi_coarse <- rep(0, length(s_semi_coarse)) + 4


ff_df <- data.frame(x = s_smooth, y = rep(0, n_smooth)+4, z = ff_eval)
...
p2fhelix <- p |> config(mathjax = 'cdn') |>
            ...
            scene = list(xaxis = list(title = list(text = "x", font = list(color = colaxnn)),  tickfont = list(color = colaxnn),  range = c(0, L)),
              ...
              #aspectmode="data",
              aspectratio = list(x = L/3, y = 5/3, z = (max(ff_eval)+1)/3),
              camera = list(eye = list(x = -6, y = -6, z = 4),
                            center = list(x = 0, y = 0, z = 0)),
              annotations = list(
                list(
...
save(p2fhelix, file = here::here("graphs6p2fhelix.Rdata"))
```

```{r, eval =TRUE, fig.height = 7, out.width = "100%"}
load(here::here("graphs6p2fhelix.Rdata"))
p2fhelix
```
...
p2 <- p |> config(mathjax = 'cdn') |>
            ...
            scene = list(xaxis = list(title = list(text = "x", font = list(color = colaxnn)),  tickfont = list(color = colaxnn),  range = c(0, L)),
              ...
              zaxis = list(title = list(text = "z", font = list(color = colaxnn)),  tickfont = list(color = colaxnn), range = c(-1, 1)),
              #aspectmode="data",
              aspectratio = list(x = L/3, y = 5/3, z = 2/3),
              camera = list(eye = list(x = -6, y = -6, z = 4),
                            center = list(x = 0, y = 0, z = 0)),
              annotations = list(
                list(
...
save(p4tadpole_arclength, file = here::here("graphs6p4tadpole_arclength.Rdata"))
```

```{r, eval =TRUE, fig.height = 7, out.width = "100%"}
load(here::here("graphs6p4tadpole_arclength.Rdata"))
p4tadpole_arclength
```
