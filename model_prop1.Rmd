...
KAPPA <- logo_graph$plot_function(X = model_for_kappa, vertex_size = 0) +
...
```{r}
p1 <- logo_graph$plot_function(X = est_cov_matrix[m1, ], vertex_size = 1, plotly = TRUE, edge_color = "black", edge_width = gsw, line_color = "#0000C8", line_width = gsw)
p2 <- logo_graph$plot_function(X = est_cov_matrix[m2, ], vertex_size = 1, plotly = TRUE, edge_color = "black", edge_width = gsw, line_color = "red", line_width = gsw, p = p1)
p3 <- logo_graph$plot_function(X = est_cov_matrix[m3, ], vertex_size = 1, plotly = TRUE, edge_color = "black", edge_width = gsw, line_color = "darkgreen", line_width = gsw, p = p2)
p_A <- p3 %>%
  config(mathjax = 'cdn') %>%
...
```{r, eval = TRUE, fig.height = 7, out.width = "100%", fig.cap = captioner("Example of covariance functions $r_i(\\cdot)$, $i=1,2,3$ for three locations with different standard deviations and practical correlation ranges on the `MetricGraph` package's logo.")}
